# Aula 13 — Epics, User Stories e Gherkin como Fonte do Spec-Kit

**Data:** 16/11/2026 | **Horário:** 11h20 às 13h00 | **Local:** Lab 207 | **Modalidade:** Presencial

[Baixar / Copiar Código Fonte da Aula](https://raw.githubusercontent.com/paulossjunior/aula-extensao/main/docs/plano-de-aula/aulas/aula-13-2026-11-16.md)

---

## Introdução

Na [Aula 09](../../plano-de-aula/aulas/aula-09-2026-10-05.md), vimos como o **Spec-Kit** transforma uma descrição de feature em spec, plano, tarefas e implementação. Na [Aula 12](../../plano-de-aula/aulas/aula-12-2026-11-09.md), vimos como o **Caveman** ajuda a reduzir ruído nas respostas do agente. Agora vamos juntar essas duas ideias em um fluxo mais realista de desenvolvimento: usar um arquivo `epics.md` com **épicos, user stories e cenários em Gherkin** como fonte de produto para orientar o agente.
...
- [Aula 09 — Desenvolvimento Orientado a Especificação](../../plano-de-aula/aulas/aula-09-2026-10-05.md)
- [Aula 12 — Comunicação Enxuta com Agentes de IA](../../plano-de-aula/aulas/aula-12-2026-11-09.md)
...
### Os dois artefatos que ligam o épico ao Spec-Kit

Ter um `epics.md` não basta: o agente precisa ser lembrado de que ele existe, e o épico precisa ser recortado antes de virar spec. Dois artefatos fazem isso.

Uma **rule de projeto** é uma instrução persistente que muda o comportamento padrão do agente. Dependendo da ferramenta, vive em `AGENTS.md`, `CLAUDE.md`, `.cursor/rules/` ou equivalente. Ela não executa nada sozinha — é um lembrete permanente: *antes de codar, olhe a fonte de produto*. As regras que importam são poucas e todas defensivas: trabalhar um épico por vez, não inventar regra de negócio fora dele, marcar ambiguidade como `[NEEDS CLARIFICATION]` em vez de adivinhar, e **preservar os cenários Gherkin como critérios de aceitação** em vez de convertê-los em detalhe de implementação.

O comando **`/epic-to-spec`** é um pré-processador. Ele lê `docs/epics.md`, encontra **um único** épico e transforma aquele trecho em uma entrada limpa para o `/speckit.specify`:

1. localizar o épico solicitado
2. ignorar todos os outros
...
!!! warning "O passo 2 é o que faz o comando valer a pena"
    Entregar o `epics.md` inteiro ao `/speckit.specify` é convidar o agente a misturar escopo de épicos diferentes. O recorte é o que mantém a spec enxuta e rastreável a uma origem só.

!!! abstract "O texto dos dois artefatos está na referência"
    A rule completa e o comando `/epic-to-spec` em Markdown, prontos para copiar, com o exemplo de saída para o Epic 01, estão em **[Referência — Spec-Kit e contexto do agente](../../referencia/spec-kit.md)**. Use nas Tarefas 2 e 3.
...
Depois comparem `tasks.md` com a DSM da EaD 07.
...
!!! note "Tarefa de Casa — Um épico completo até implementação"
    Este é o insumo da **demonstração de medição da Aula 14**, em que a mesma user story será executada de novo, com uma crew, para comparação. Cada grupo deve:
