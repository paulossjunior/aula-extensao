...
**Data:** 05/10/2026 | **Horário:** 11h20 às 13h00 | **Local:** Lab 207 | **Modalidade:** Presencial

[⬇️ Baixar / Copiar Código Fonte da Aula](https://raw.githubusercontent.com/paulossjunior/aula-extensao/main/docs/plano-de-aula/aulas/aula-09-2026-10-05.md)

---

## Introdução

Na [sessão EaD 07](../../plano-de-aula/aulas/ead-07-2026-09-22.md) cada grupo saiu com três artefatos importantes: um backlog inicial em formato Scrum, uma lista de **features fim a fim** e uma **DSM** que organiza dependências e ordem de implementação. Esses artefatos respondem bem a duas perguntas: *o que entregar primeiro* e *em que ordem construir*. Falta agora responder a uma terceira: *como descrever uma feature de modo que um agente de IA gere código previsível, e não algo aproximadamente parecido com o que o grupo imaginou*.
...
- [EaD 07 — Scrum, Features Fim a Fim e DSM](../../plano-de-aula/aulas/ead-07-2026-09-22.md)
...
A instalação usa o gerenciador `uv`, e o `specify init` prepara o projeto com uma pasta `.specify/`. A partir daí, o trabalho acontece em **fases, cada uma disparada por um slash-command dentro do agente** — e a ordem delas é a tese do SDD em forma de ferramenta:

| Fase | Comando | Responde |
|------|---------|----------|
| Princípios | `/speckit.constitution` | O que este produto nunca pode violar? |
| Especificação | `/speckit.specify` | **O quê** e **por quê**, sem decidir stack |
| Clarificação | `/speckit.clarify` | O que ainda está ambíguo? |
| Plano | `/speckit.plan` | **Como**: stack, arquitetura, contratos, modelo de dados |
| Tarefas | `/speckit.tasks` | Em que ordem, e o que dá para paralelizar |
| Implementação | `/speckit.implement` | O código |

Repare no que a ordem impede: não há como decidir stack antes de ter dito o que a feature faz. No produto de envio de comprovantes, a constitution diria algo como *"todo envio gera protocolo rastreável com data e hora"*, e só várias fases depois alguém escolheria o banco.
...
!!! abstract "Consulte durante as tarefas"
    Comandos de instalação, o que cada fase grava em disco, os comandos de validação (`/speckit.analyze`, `/speckit.checklist`) e as três camadas de contexto do agente estão em **[Referência — Spec-Kit e contexto do agente](../../referencia/spec-kit.md)**.

### Da DSM e features fim a fim para a especificação

O trabalho da EaD 07 alimenta diretamente o que vamos fazer hoje:
...
```mermaid
flowchart TD
    A[Problema validado<br/>Aulas 01–04] --> B[Backlog + Features fim a fim + DSM<br/>EaD 07]
...
3. **UX** — quais expectativas de experiência? Por exemplo: "o aluno precisa conseguir fotografar e enviar o comprovante pelo celular, sem passar por computador".
...
Cada grupo escolhe **uma única feature** do backlog produzido na EaD 07 — preferencialmente uma feature fim a fim de tamanho médio, nem trivial nem gigante.
...
3. Pegar a **DSM** da EaD 07 e comparar: a ordem que o agente propôs respeita as dependências mapeadas pelo grupo? Onde diverge?
...
A próxima virada depende de uma coisa que ainda não fizemos: **garantir que o agente conheça o projeto** antes de escrever qualquer linha. Spec boa num repositório que o agente não entende ainda gera código fora do padrão do time. É o que veremos na próxima aula **(19/10/2026)**, com `AGENTS.md`, a constitution e as skills.
