...
A nota são **100 pontos**, e a aprovação exige **60 ou mais**.

| Frente | Valor | O que é |
|--------|-------|---------|
| Código próprio | 25 pontos | Repositório individual do aluno, comprovando a aplicação do conteúdo |
| Mini-curso | 25 pontos | Vídeo individual, no canal do aluno, explicando o próprio código |
| Trabalho em grupo | 50 pontos | Cinco itens de 10: produto entregue, em produção, documentação, código e apresentação |
| Observabilidade do produto | **+10 pontos** | Opcional: health check, log estruturado e alerta automático |
| Entrevista | **0 ou 1** | Porteira dos 50 pontos do grupo — é preciso passar nos quatro checkpoints |

```text
NF = min(100, COD + CUR + (GRP + EXT) x ENT)
```

Os checkpoints acontecem em **31/08, 28/09, 26/10 e 30/11**, nas segundas de apresentação. São individuais: o aluno senta na máquina e resolve uma tarefa pequena sobre o repositório dele ou do grupo. **É preciso passar nos quatro** para receber os pontos do trabalho em grupo.

!!! danger "Quem não souber trabalhar sobre o próprio código, perde os pontos do grupo"
    Produto em produção não garante nota a quem não consegue sustentar o que foi entregue em seu nome. É também por isso que o uso de agentes de IA é liberado: a ferramenta escreve, mas quem responde é você.

    O produto **não entregue** tem o mesmo efeito, com alcance maior: zera os 50 pontos para o grupo inteiro.
